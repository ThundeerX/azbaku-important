...
(function(){
  ...
  function updateButtons(){
    document.querySelectorAll('.lang-btn[data-lang]').forEach(function(b){
      var own = b.dataset.lang;
      var isActive = (lang === own);
      var show = isActive ? 'az' : own;
      var f = FLAGS[show];
      b.innerHTML = '<img src="'+f.img+'" alt="'+f.label+'">'+f.label;
      b.classList.toggle('active', isActive);
      b.title = isActive ? 'Azərbaycanca' : (own === 'en' ? 'English' : 'Русский');
    });
  }
  ...
  function buildSwitcher(){
    ...
    wrap.querySelectorAll('.lang-btn').forEach(function(b){
      b.addEventListener('click', function(){
        var target = (lang === b.dataset.lang) ? 'az' : b.dataset.lang;
        applyLang(target);
      });
    });
